...
<script>
                        ...
                        if(response.status == 200){
							...
							$.each(response.responseJSON.data, function(k,v){
								...
								for(var y = end; y <= year; y++){ 
									...
									if(item == undefined){
										id = null;
										tbody += '<td class="text-center"><a class="add btn list-icons-item" data-id="'+id+'" data-year="'+y+'" data-value="'+null+'" data-name="'+v.uraian+'">&nbsp;</a></span></td>';	
									
									}else{
										if(item.value == 0) item.value = '';
										id = item.id;
										tbody += '<td class="text-center"><a class="add btn list-icons-item" data-id="'+id+'" data-year="'+y+'" data-value="'+item.value+'" data-name="'+v.uraian+'">'+item.value+'</a></td>';
	...
	$( ".add" ).on( "click", function(event) {
		...
		if(value == null){
		$('#title-add').html('Insert data makro '+year);
		}else{
		$('#title-add').html('Update data makro '+year);		
		}
		$('#name-add').html(name);
	...
	$( ".opt" ).on( "click", function(event) {
		...
		$('#title-insert').html('Update Uraian Makro Provinsi ');
		$('#name-insert').html('Anda akan merubah uraian data makro berikut');
		$("input[name=id]").val(id);
		$("input[name=name]").val(name);
