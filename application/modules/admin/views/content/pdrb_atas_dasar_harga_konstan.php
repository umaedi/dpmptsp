					...
					<table id="table" class="table table-bordered table-striped table-hover">
						<thead>
							<tr>
								...
								<th class="text-center h4" rowspan="2">Sektor</th>
								<th class="text-center h5" id="table-title" colspan="4">Tahun <i class="small">( dalam jutaan rupiah )</i></th>
...
<script>
							...
							$.each(response.responseJSON.data, function(k,v){
								...
								for(var y = end; y <= year; y++){ 
									...
									if((item == undefined) || (item.value == 0)){
										...
										id = item.id;
										tbody += '<td class="text-right"><a class="add btn list-icons-item '+y+'" data-id="'+id+'" data-year="'+y+'" data-value="'+item.value+'" data-name="'+v.uraian+'">'+my_number(item.value)+'</a></td>';
	...
	for(var i = end; i <= year; i++){
		...
		$("."+i).each(function() {
		 ...
		 a += parseFloat(value);
			...
			listTotal += "<th class='text-right h4'>"+my_number(parse(a))+"</th>";
	}
		var n = 0;
		var b = 0;
		$(".avg").each(function() {
		 value = $(this).data("value");
		 if((value != null) && (value != '') && (value != 0)){ n += 1;}
		 if(value == null) value = 0;
		 if(value == '') value = 0;
		 b += parseFloat(value);
	...
	$( ".add" ).on( "click", function(event) {
		...
		if(value == null){
		$('#title-add').html('Insert PDRB ADHK '+year);
		}else{
		$('#title-add').html('Update PDRB ADHK '+year);		
		}
		$('#name-add').html("Sektor "+name);
	...
	$( ".opt" ).on( "click", function(event) {
		...
		var name = $(this).data("name");
		$('#title-insert').html('Update PDRB ADHK');
	...
	function insert(){
		...
		$("input[name=year]").val(year);
		$('#modal_insert').find('#form-insert')[0].reset();
		$('#title-insert').html('Insert Uraian PDRB ADHK');
		$('#name-insert').html('Anda akan menambahkan uraian PDRB ADHK baru');
		$("#modal_insert").modal("show");
