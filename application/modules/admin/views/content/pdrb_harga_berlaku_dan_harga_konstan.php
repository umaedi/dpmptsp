...
<script> 
	var sub 	= "admin/";
	var param = 'pdrb_harga_berlaku_dan_harga_konstan';
	function loadData(){
		$('#table tbody').html('');  
	...
	$( ".opt" ).on( "click", function(event) {
		...
		var harga_berlaku 	= $(this).data("harga_berlaku");
		var harga_konstant 	= $(this).data("harga_konstant");
		var persentase = $(this).data("persentase");
		$('#title-insert').html('Update PDRB Berlaku & Konstan '+tahun);
		$('#name-insert').html('Anda akan merubah tahun harga berlaku & harga konstan berikut');
		$("input[name=id]").val(id);
		$("select[name=year2]").val(tahun);
		$("input[name=harga_berlaku]").val(harga_berlaku);
		$("input[name=harga_konstant]").val(harga_konstant);
	...
	function insert(){
		...
		$("input[name=id]").val('');
		$('#modal_insert').find('#form-insert')[0].reset();
		$('#title-insert').html('Insert PDRB Berlaku & Konstan');
		$('#name-insert').html('Anda akan menambahkan tahun BDRB Berlaku & Konstan baru');
		$("#modal_insert").modal("show");
		$('#modal_insert').on('shown.bs.modal', function () {
		$('select[name=year2]').focus();	
