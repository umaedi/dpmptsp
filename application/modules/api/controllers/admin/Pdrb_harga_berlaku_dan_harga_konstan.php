...
class Pdrb_harga_berlaku_dan_harga_konstan extends MY_Api {
    function __construct()
    {
        // Construct the parent class
        parent::__construct();
         
		$this->load->model('admin/m_pdrb_harga_berlaku_dan_harga_konstan');
	...
	public function list_get()
    {
		...
		$offset		= ($int * $perpage);
		$limit		= $perpage;
		$result 	= $this->m_pdrb_harga_berlaku_dan_harga_konstan->get_all($limit, $offset, $end, $year);
		$total		= $this->m_pdrb_harga_berlaku_dan_harga_konstan->count_all($end, $year);
		$pagecount	=  ceil($total/$perpage);
		$item		=  array();
		$cagr		= 0;
		foreach ($result as $items){
			if(($items['harga_berlaku'] != '') AND ($items['harga_konstant'] != '' ) AND ($items['harga_berlaku'] != 0) AND ($items['harga_konstant'] != 0 )){
			$getold		= $this->m_pdrb_harga_berlaku_dan_harga_konstan->getold(((int)$items['tahun']-1));
	...
	public function view_get()
    {
		...
		$id 		= $this->get('id');
		$result 		= $this->m_pdrb_harga_berlaku_dan_harga_konstan->get_one($id);
		$data['data'] = $users;
			// Check if the users data store contains users (in case the database result returns NULL)
            if ($result)
            {
    ...
    public function save_post($id =NULL)
    {
			...
			$check = $this->m_pdrb_harga_berlaku_dan_harga_konstan->get_one($id);
			...
			if ($this->form_validation->run() == TRUE) 
			{
				...
				$getold		= $this->m_pdrb_harga_berlaku_dan_harga_konstan->getold(((int)$tahun-1));
				...
				if(!$id)
				{ 
				$return = $this->m_pdrb_harga_berlaku_dan_harga_konstan->save($data); 
				if($return == true){
				...
				$return = $this->m_pdrb_harga_berlaku_dan_harga_konstan->update($id, $data); 
				if($return == true){
    ...
    public function delete_get()
    {
        ...
        $return = $this->m_pdrb_harga_berlaku_dan_harga_konstan->destroy($id);
		if($return == true){
        $this->set_response([
							'status' => 'ok',
