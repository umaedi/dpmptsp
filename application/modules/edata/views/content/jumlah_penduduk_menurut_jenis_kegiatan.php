...
<script>
	var today = new Date();
	var param = 'jumlah_penduduk_berumur_15_tahun';
	function loadData(){
                    ...
                    complete: function(response){                
                        if(response.status == 200){
							...
							$.each(response.responseJSON.data, function(k,v){
								no++;
								tbody += '<tr>';
								tbody += '<td class="text-center small"><a class="opt btn list-icons-item " data-id="'+v.id+'" data-name="'+v.jenis_kegiatan+'">'+no+'.</a></td>';
