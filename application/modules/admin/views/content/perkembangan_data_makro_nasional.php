...
<script>
                    ...
                    complete: function(response){                
                        if(response.status == 200){
							...
							$.each(response.responseJSON.data, function(k,v){
								no++;
								tbody += '<tr>';
								tbody += '<td class="text-center small"><a class="opt btn list-icons-item btn-outline-warning" data-id="'+v.id+'" data-name="'+v.name+'" data-type="'+v.type+'">'+no+'.</a></td>';
							...
							$.each(response.responseJSON.data2, function(k,v){
								no++;
								tbody2 += '<tr>';
								tbody2 += '<td class="text-center small"><a class="opt btn list-icons-item btn-outline-warning" data-id="'+v.id+'" data-name="'+v.name+'" data-type="'+v.type+'">'+no+'.</a></td>';
	...
	$( ".opt" ).on( "click", function(event) {
		...
		var name = $(this).data("name");
		var type = $(this).data("type");
		$('#title-insert').html('Update '+type+' ');
		$('#name-insert').html('Anda akan merubah '+type+' berikut');
		$("input[name=id]").val(id);
		$("input[name=name]").val(name);
