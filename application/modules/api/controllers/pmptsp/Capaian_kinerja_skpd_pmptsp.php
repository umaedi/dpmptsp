...
class Capaian_kinerja_skpd_pmtsp extends MY_Api {
    function __construct()
    {
        // Construct the parent class
        parent::__construct();
         
		$this->load->model('pmtsp/m_capaian_kinerja_skpd_pmtsp');
	...
	public function list_get()
    {
		...
		$offset		= ($int * $perpage);
		$limit		= $perpage;
		$result 	= $this->m_capaian_kinerja_skpd_pmtsp->get_all($periode, $limit, $offset);
		$total		= $this->m_capaian_kinerja_skpd_pmtsp->count_all($periode);
		...
		foreach ($result as $items){
					...
					$item[] = array(
									...
									"satuan" 		=> $items['satuan'],
									"year" 			=> $this->m_capaian_kinerja_skpd_pmtsp->get_items($periode, $year, $end, $items['tujuan']),
								);
	...
	public function view_get()
    {
		...
		$id 		= $this->get('id');
		$users 		= $this->m_capaian_kinerja_skpd_pmtsp->get_one($id);
		$data['data'] = $users;
			// Check if the users data store contains users (in case the database result returns NULL)
            if ($users)
            {
    ...
    public function save_post($id =NULL)
    {
			...
			$check = $this->m_capaian_kinerja_skpd_pmtsp->get_one($id);
			...
			if($this->post('type') == 'target'){ 
				...
				if(!$id)
				{ 
				$return = $this->m_capaian_kinerja_skpd_pmtsp->save($data); 
				if($return == true){
				...
				$return = $this->m_capaian_kinerja_skpd_pmtsp->update($id, $data); 
				if($return == true){
	...
	public function insert_post($id = NULL)
    {
			...
			if ($this->form_validation->run() == TRUE) 
			{
				...
				$return = $this->m_capaian_kinerja_skpd_pmtsp->update_name($id, $data); 
				if($return == true){
    ...
    public function delete_get()
    {
        ...
        $return =$this->m_capaian_kinerja_skpd_pmtsp->destroy($id);
		if($return == true){
        $this->set_response([
							'status' => 'ok',
