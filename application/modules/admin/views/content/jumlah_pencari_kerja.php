			...
			<span id="alert"></span>
					...
					<table id="table" class="table table-bordered">
						<thead>
							<tr>
								...
								<th class="text-center h4" rowspan="2">Tingkat Pendidikan</th>
								<th class="text-center h5" id="table-title" colspan="5">Jumlah Pencari Kerja</th>
				...
				<div class="card">
					<div class="card-header mb-2 header-elements-inline">
						<h5 class="card-title">Grafik <?php echo $halaman; ?> <span class="title"></span></h5>
						<div class="header-elements">
							<div class="list-icons">
		                		<a class="list-icons-item" data-action="collapse"></a>
...
<script>
                    ...
                    complete: function(response){                
                        if(response.status == 200){
							...
							$.each(response.responseJSON.data, function(k,v){
								no++;
								tbody += '<tr>';
								tbody += '<td class="text-center small"><a class="opt btn list-icons-item btn-outline-warning" data-id="'+v.id+'" data-name="'+v.tingkat_pendidikan+'">'+no+'.</a></td>';
								tbody += '<td class="text-left">'+v.tingkat_pendidikan+'</td>';
								for(var y = end; y <= year; y++){
									var item = v.year[y]; 
									if(item == undefined){
										id = null;
										tbody += '<td class="text-center"><a class="add btn list-icons-item '+y+'" data-id="'+id+'" data-year="'+y+'" data-value="'+null+'" data-name="'+v.tingkat_pendidikan+'">&nbsp;</a></span></td>';	
									
									}else{
										if(item.value == 0) item.value = '';
										id = item.id;
										tbody += '<td class="text-center"><a class="add btn list-icons-item '+y+'" data-id="'+id+'" data-year="'+y+'" data-value="'+item.value+'" data-name="'+v.tingkat_pendidikan+'">'+item.value+'</a></td>';
	...
	$( ".add" ).on( "click", function(event) {
		...
		if(value == null){
		$('#title-add').html('Insert Jumlah Penduduk '+year);
		}else{
		$('#title-add').html('Update Jumlah Penduduk '+year);		
		}
		$('#name-add').html("Tingkat Pendidikan "+name);
	...
	$( ".opt" ).on( "click", function(event) {
		...
		var name = $(this).data("name");
		$('#title-insert').html('Update Tingkat Pendidikan');
		$('#name-insert').html('Anda akan merubah tingkat pendidikan berikut');
		$("input[name=id]").val(id);
		$("input[name=name]").val(name);
	...
	function insert(){
		...
		$("input[name=year]").val(year);
		$('#modal_insert').find('#form-insert')[0].reset();
		$('#title-insert').html('Insert Tingkat Pendidikan');
