...
class Rata_rata_lama_sekolah extends MY_Api {
    function __construct()
    {
        // Construct the parent class
        parent::__construct();
        
		$this->load->model('admin/m_rata_rata_lama_sekolah');
	...
	public function list_get()
    {
		...
		$offset		= ($int * $perpage);
		$limit		= $perpage;
		$result 	= $this->m_rata_rata_lama_sekolah->get_all($limit, $offset);
		$total		= $this->m_rata_rata_lama_sekolah->count_all();
		...
		foreach ($result as $items){
					$item[] = array(
									"id" 					=> $items['id'],
									"uraian" 				=> $items['uraian'],
									"year" 					=> $this->m_rata_rata_lama_sekolah->get_items($year, $end, $items['uraian']),
	...
	public function view_get()
    {
		...
		$id 		= $this->get('id');
		$users 		= $this->m_rata_rata_lama_sekolah->get_one($id);
		$data['data'] = $users;
			// Check if the users data store contains users (in case the database result returns NULL)
            if ($users)
            {
    ...
    public function save_post($id =NULL)
    {
			...
			$check = $this->m_rata_rata_lama_sekolah->get_one($id);
			...
			if ($this->form_validation->run() == TRUE) 
			{
				...
				if(!$id)
				{ 
				$return = $this->m_rata_rata_lama_sekolah->save($data); 
				if($return == true){
				...
				$return = $this->m_rata_rata_lama_sekolah->update($id, $data); 
				if($return == true){
	...
	public function insert_post($id = NULL)
    {
			...
			$config = array(
						array(
							'field' => 'name',
							'label' => 'name',
							'rules' => 'trim|required|is_unique[tbl_rata_rata_lama_sekolah.uraian]'
							),				
					  );
			...
			if ($this->form_validation->run() == TRUE) 
			{
				...
				$return = $this->m_rata_rata_lama_sekolah->update_name($id, $data); 
				if($return == true){
    ...
    public function delete_get()
    {
        ...
        $return = $this->m_rata_rata_lama_sekolah->destroy($id);
		if($return == true){
        $this->set_response([
							'status' => 'ok',
