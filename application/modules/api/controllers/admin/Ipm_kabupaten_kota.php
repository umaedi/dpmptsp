...
class Ipm_kabupaten_kota extends MY_Api {
    function __construct()
    {
        // Construct the parent class
        parent::__construct();
        
		$this->load->model('admin/m_ipm_kabupaten_kota');
	...
	public function list_get()
    {
		...
		$offset		= ($int * $perpage);
		$limit		= $perpage;
		$result 	= $this->m_ipm_kabupaten_kota->get_all($limit, $offset);
		$total		= $this->m_ipm_kabupaten_kota->count_all();
		$pagecount	= ceil($total/$perpage);
		$item		=  array();
		foreach ($result as $items){
				$get = $this->m_ipm_kabupaten_kota->get_items($year, $end, $items['uraian']);
				$cagr = 0;
				$t	  = 0;
				for ($i = $end; $i <= $year; $i++){
					if(array_key_exists($i, $get)){
					$ends 		= $this->m_ipm_kabupaten_kota->get_end_value($i, $items['uraian']);	
					$start  	= $get[$i]['value'];
	...
	public function view_get()
    {
		...
		$id 		= $this->get('id');
		$users 		= $this->m_ipm_kabupaten_kota->get_one($id);
		$data['data'] = $users;
			// Check if the users data store contains users (in case the database result returns NULL)
            if ($users)
            {
    ...
    public function save_post($id =NULL)
    {
			...
			$check = $this->m_ipm_kabupaten_kota->get_one($id);
			...
			if ($this->form_validation->run() == TRUE) 
			{
				...
				if(!$id)
				{ 
				$return = $this->m_ipm_kabupaten_kota->save($data); 
				if($return == true){
				...
				$return = $this->m_ipm_kabupaten_kota->update($id, $data); 
				if($return == true){
	...
	public function insert_post($id = NULL)
    {
			...
			$config = array(
						array(
							'field' => 'name',
							'label' => 'name',
							'rules' => 'trim|required|is_unique[tbl_ipm_kabupaten_waykanan.uraian]'
							),				
					  );
			...
			if ($this->form_validation->run() == TRUE) 
			{
				...
				$return = $this->m_ipm_kabupaten_kota->update_name($id, $data); 
				if($return == true){
    ...
    public function delete_get()
    {
        ...
        $return = $this->m_ipm_kabupaten_kota->destroy($id);
		if($return == true){
        $this->set_response([
							'status' => 'ok',
