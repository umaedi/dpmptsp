...
class M_jumlah_penduduk_menurut_kecamatan extends CI_Model 
{
	public $primary		= 'id';
    public $tbl_name	= 'tbl_jumlah_penduduk_menurut_kecamatan';
	...
	public function get_end_value($y, $like) 
    {
		...
		$this->db->where('jumlah_penduduk != 0', null);
    ...
    {       
        ...
        $result = $this->db->get($this->tbl_name);
		$result = $result->row_array();
		$name 	= $result['kecamatan'];
		
		$this->db->where('kecamatan', $name);
        $this->db->update($this->tbl_name, $data);
		$log = array("user_id" 		=> $this->user_data->id, "description" 		=> 'Memperbaharui '.$this->tbl_name.' nama '.json_encode($name).' menjadi '.json_encode($data),);
		$this->db->insert('users_log', $log);
		return true;
    ...
    {       
		...
		$log = array("user_id" 		=> $this->user_data->id, "description" 		=> 'Menghapus '.$this->tbl_name.' item '.json_encode($name),);
		$this->db->insert('users_log', $log);
		return true;
