...
<script>
                    ...
                    complete: function(response){                
                        if(response.status == 200){
							...
							$.each(response.responseJSON.data, function(k,v){
								no++;
								tbody += '<tr>';
								tbody += '<td class="text-center small"><a class="opt btn list-icons-item " data-id="'+v.id+'" data-name="'+v.uraian+'">'+no+'.</a></td>';
								...
								if(v.avg == 0) v.avg = '';
								tbody += '<td class="text-right avg" data-value="'+v.avg+'">'+v.avg+'</td>';
								tbody += "</tr>";
	...
	for(var i = end; i <= year; i++){
		...
		b = parseFloat(b) / n;
		listTotal += "<th class='text-right h4'>"+parse(b)+"</th>";
