...
class Aspek_pelayanan_skpd_pmptsp extends MY_Api {
    function __construct()
    {
        // Construct the parent class
        parent::__construct();
       
		$this->load->model('pmptsp/m_aspek_pelayanan_skpd_pmptsp');
	...
	public function list_get()
    {
		...
		$offset		= ($int * $perpage);
		$limit		= $perpage;
		$result 	= $this->m_aspek_pelayanan_skpd_pmptsp->get_all($periode, $limit, $offset);
		$total		= $this->m_aspek_pelayanan_skpd_pmptsp->count_all($periode);
		...
		foreach ($result as $items){
					$item[] = array(
									...
									"uraian" 				=> $items['uraian'],
									"satuan" 				=> $items['satuan'],
									"year" 					=> $this->m_aspek_pelayanan_skpd_pmptsp->get_items($periode, $year, $end, $items['uraian']),
	...
	public function view_get()
    {
		...
		$id 		= $this->get('id');
		$users 		= $this->m_aspek_pelayanan_skpd_pmptsp->get_one($id);
		$data['data'] = $users;
			// Check if the users data store contains users (in case the database result returns NULL)
            if ($users)
            {
    ...
    public function save_post($id =NULL)
    {
			...
			$check = $this->m_aspek_pelayanan_skpd_pmptsp->get_one($id);
			...
			if ($this->form_validation->run() == TRUE) 
			{
				$data = array(
					...
					'aspek' => strip_tags($this->post('aspek', TRUE)),
				...
				if(!$id)
				{ 
				$return = $this->m_aspek_pelayanan_skpd_pmptsp->save($data); 
				if($return == true){
				...
				$return = $this->m_aspek_pelayanan_skpd_pmptsp->update($id, $data); 
				if($return == true){
	...
	public function insert_post($id = NULL)
    {
			...
			if ($this->form_validation->run() == TRUE) 
			{
				...
				$return = $this->m_aspek_pelayanan_skpd_pmptsp->update_name($id, $data); 
				if($return == true){
    ...
    public function delete_get()
    {
        ...
        $return = $this->m_aspek_pelayanan_skpd_pmptsp->destroy($id);
		if($return == true){
        $this->set_response([
							'status' => 'ok',
