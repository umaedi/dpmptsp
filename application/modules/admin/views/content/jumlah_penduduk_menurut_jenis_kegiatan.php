					...
					<table id="table" class="table table-bordered">
						<thead>
							<tr>
								...
								<th class="text-center h5" id="table-title" colspan="4">Jumlah Penduduk ( % )</th>
...
<script>
                    ...
                    complete: function(response){                
                        if(response.status == 200){
							...
							$.each(response.responseJSON.data, function(k,v){
								no++;
								tbody += '<tr>';
								tbody += '<td class="text-center small"><a class="opt btn list-icons-item btn-outline-warning" data-id="'+v.id+'" data-name="'+v.jenis_kegiatan+'">'+no+'.</a></td>';
								tbody += '<td class="text-left">'+v.jenis_kegiatan+'</td>';
								for(var y = end; y <= year; y++){
									var item = v.year[y]; 
									if(item == undefined){
										id = null;
										tbody += '<td class="text-center"><a class="add btn list-icons-item '+y+'" data-id="'+id+'" data-year="'+y+'" data-value="'+null+'" data-name="'+v.jenis_kegiatan+'">&nbsp;</a></span></td>';	
									
									}else{
										if(item.value == 0) item.value = '';
										id = item.id;
										tbody += '<td class="text-center"><a class="add btn list-icons-item '+y+'" data-id="'+id+'" data-year="'+y+'" data-value="'+item.value+'" data-name="'+v.jenis_kegiatan+'">'+item.value+'</a></td>';
	...
	$( ".opt" ).on( "click", function(event) {
		...
		var name = $(this).data("name");
		$('#title-insert').html('Update Kecamatan');
		$('#name-insert').html('Anda akan merubah kecamatan berikut');
		$("input[name=id]").val(id);
		$("input[name=name]").val(name);
